...
class DeployDataProduct():
    ...
    def run(self):
        ...
        output_filename = "/tmp/" + datetime.date.today().strftime("%Y-%m-%d") + "-deploys-vera-since-2021.parquet"
    ...
    def get_deploydata_from_vera(self):
        start = time.time()
        days_since_cutoff = (datetime.datetime.now().date() - datetime.date(2023, 1, 1)).days
    ...
    def transform(self, csv_text, parquet_filename):
        ...
        logger.info("lengt of dataframe" + str(len(df)))
